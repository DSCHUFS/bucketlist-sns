...
exports.likeAPI = async(req, res) => {
    try {
        const user_id = res.user_id // header에 user token
        const bucket_id = Number(req.params.bucket_id) // 좋아요 누른 버킷id
        const url = req.baseUrl
        const like = (url === '/like')? true : false
        ...
        res.status(200).json({'msg' : `${url.substr(1)} bucket`})
    } catch(e) {
        console.log(e)
        res.status(400)
    }
